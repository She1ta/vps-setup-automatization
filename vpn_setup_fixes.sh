...
# --- 3. PERFORMANCE: ZRAM, SWAP & LIMITS ---
status_working "Optimizing RAM, Swap & System Limits"
apt-get install -y zram-tools >> "$LOG_FILE" 2>&1
echo -e "ALGO=zstd\nPERCENT=60" > /etc/default/zramswap
systemctl restart zramswap >> "$LOG_FILE" 2>&1

# Add 2GB SSD Swap if it doesn't exist
if [ ! -f /swapfile ]; then
    fallocate -l 2G /swapfile || dd if=/dev/zero of=/swapfile bs=1M count=2048 >> "$LOG_FILE" 2>&1
    chmod 600 /swapfile
...
# NOTE: The destination-unreachable drop was intentionally REMOVED here.
# Dropping it breaks MTU discovery, which is why your Web UI was stalling after 30s.
...
echo -e "  ${GREEN}${CHECK}${NC} Admin Pass:    ${BLUE}$USER_PASS${NC}"
